...
; *** NEED  TO REPLACE GPIO_PORTA... WITH DC ***
writecommand
	LDR R1,=SSI0_SR_R
	; Steps 1&2: Polling if bit 4 of status register is clear
check1cmd 
	LDR R2, [R1]
	ANDS R2, #0x10
	BNE check1cmd
	; Step 3: Clearing PA6 (D/C)
	LDR R1,=GPIO_PORTA_DATA_R
	LDR R2, [R1]
	BIC R2, #0x40 ;this might be the wrong bit, check value of this reg if code malfunctions
	STR R2, [R1]
	;Step 4: Writing to Device Register
	LDR R1,=SSI0_DR_R
	STR R0, [R1]
	; Step 5: Polling for busy flag of status register
check2cmd
	LDR R1,=SSI0_SR_R
	LDR R2, [R1]
	ANDS R2, #0x10
...
;4) Write the 8-bit data to SSI0_DR_R
	...
	LDR R1,=SSI0_SR_R
	; Steps 1&2: Polling if bit 1 of status register is clear
check1data
	LDR R2, [R1]
	ANDS R2, #0x02
	BEQ check1data
	; Step 3: Setting D/C to 1
	LDR R1,=GPIO_PORTA_DATA_R
	LDR R2, [R1]
	ORRS R2, #0x40 ;this might be the wrong bit, check value of this reg if code malfunctions
	STR R2, [R1]
	; Step 4: Writing data to device register
	LDR R1,=SSI0_DR_R
	STRB R0, [R1]
